...
- **Typing discipline as a hard rule** — "always use static typing" paired with an explicit ban on `from __future__ import annotations`. You state the floor and the prohibition together. (seen across several private repos)
...
- **Workflow automation rules** — "commit (and push) when a task is complete," and in one repo "repeat what I asked and summarize what you did." These shape the agent's loop, not the source. (seen across several private repos)
- **Overloaded shorthand verbs expanded into rituals** — a single word stands for a fixed multi-step sequence the agent must run in full. "Merge" inside a worktree means commit → rebase `main` → `--ff-only` merge → remove the worktree and branch → push. You encode the expansion once so the verb is unambiguous. (seen in `JasonLo/cheatsheet:CLAUDE.md`)
- **Platform overrides that invert the defaults** — the CircuitPython repo bans typing, f-strings, pathlib, and `|` unions and mandates `%` formatting, because the runtime can't support the usual stack. The pattern: when the target platform breaks a default preference, restate the inversion explicitly so the agent doesn't apply the global habit. (seen in `JasonLo/claude-quota-esp32:CLAUDE.md`)
...
- **"Ban `from __future__ import annotations` as a style choice"** → **"The language caught up — the import now buys nothing."** Your ban is correct, but frame the *why*: on Python 3.14 deferred annotation evaluation (PEP 649/749) is built in, and `X | Y` unions have worked at runtime since 3.10. The future-import was a forward-compat crutch; on a modern floor it only adds noise. Keeping the rule is right; understanding it as "obsolete crutch" not "personal taste" is what makes it portable. (seen across several private repos)
- **"Pin a Python version per repo"** → **"State the real floor, or track one current baseline."** The scattered `3.12+`/`3.13+`/`3.14+` floors are mostly stale copy-paste, not three deliberate decisions. Each distinct number implies a compatibility commitment you didn't actually make. Either declare the genuine minimum a repo needs (a library) or track the current stable everywhere (an app) — don't let copy-paste manufacture phantom constraints. (seen across the preference blocks above)
...
- **"`merge` is a literal `git merge`"** → **"A workflow verb is shorthand for the integration ritual you actually want."** Taken literally, "merge" would run one command and stop. What you mean inside a worktree is the whole sequence — commit, rebase `main`, `--ff-only` merge, tear down the worktree and branch, push — so the repo stays linear and worktrees don't accumulate. The mental shift: spell out the expansion in CLAUDE.md so the agent runs the ritual, not the dictionary definition. (seen in `JasonLo/cheatsheet:CLAUDE.md`)
