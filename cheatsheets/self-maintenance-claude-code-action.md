...
- The scheduled prompt is a full audit protocol: orient on named source files → find discrepancies/violations → apply only the *mechanical, verifiable* fixes → open a PR, or no-op when clean. (`JasonLo/jasonlo.dev:.github/workflows/doc-cleaner-claude.yml`)
...
- Everything is on `anthropics/claude-code-action@v1` with auth via `claude_code_oauth_token` (OAuth token in a secret), not an Anthropic API key.
- `additional_permissions: actions: read` is added wherever Claude needs to read CI results on a PR; `id-token: write` is present on every job for OIDC. (`JasonLo/uw-s3:.github/workflows/claude.yml`)
- `claude_args` is the single knob for behavior — `--max-turns`, `--allowedTools` — passed as a multiline string; one repo already narrows tools to `Bash(gh:*),Read,Glob,Edit,Write`. (`JasonLo/skill-sommelier:.github/workflows/claude.yml`)
...
- **"Codebase orientation belongs in the workflow prompt"** → **the action runs Claude Code, which already reads `CLAUDE.md` — keep durable policy there, keep the workflow to the trigger + task.** The doc-cleaner prompt hand-lists `astro.config.mjs`, `package.json`, `content.config.ts` to read; that orientation is exactly what `CLAUDE.md` is for, and your local CLI and the interactive `@claude` bot get it for free from the same file. Workflow YAML is orchestration, not a knowledge base. (`JasonLo/jasonlo.dev:.github/workflows/doc-cleaner-claude.yml`)
- **"A maintenance agent sees the whole repo"** → **a shallow checkout silently blinds it to history.** `actions/checkout` defaults to one commit; the current prompts only inspect present state, so depth-1 is fine today — but the moment a self-maintenance task says "what changed since last week" or "audit recent commits," it sees nothing and reports clean. Set `fetch-depth: 0` whenever the agent reasons about change over time. (`JasonLo/jasonlo.dev:.github/workflows/doc-cleaner-claude.yml`)
